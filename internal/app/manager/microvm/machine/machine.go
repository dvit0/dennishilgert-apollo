...
// NewMachineInstance create a new MachineInstance.
...
func (m *MachineInstance) ShutdownAndDestroy(parentCtx context.Context) error {
	log.Debugf("shutting down firecracker vm: %s", m.Cfg.VmId.String())
	...
	// check every 500cms if the machine is still running
	...
	for {
		...
		case <-ticker.C:
			if !m.IsRunning() {
				log.Debugf("firecracker vm has been shut down gracefully: %s", m.Cfg.VmId.String())
				return nil
			}
		case <-ctx.Done():
			log.Debugf("force stopping firecracker vm: %s", m.Cfg.VmId.String())

			if err := m.Machine.StopVMM(); err != nil {
				log.Errorf("failed to force stop the firecracker vm: %v", err)
				return err
			} else {
				log.Warnf("firecracker vm has been stopped forcefully: %s", m.Cfg.VmId.String())
				return nil
			}
		}
	}
...
func (m *MachineInstance) establishConnection(ctx context.Context) error {
	log.Debugf("establishing a connection to agent in firecracker vm: %s", m.Cfg.VmId.String())

	if m.ClientConn != nil {
		if m.ConnectionAlive() {
			log.Infof("connection alive - no need to establish a new connection to agent in firecracker vm: %s", m.Cfg.VmId.String())
			return nil
		}
	...
	for i := 0; i < (retrySeconds * retriesPerSecond); i++ {
		...
		select {
		case <-ctx.Done():
			log.Errorf("failed to establish a connection to the agent in firecracker vm: %s", m.Cfg.VmId.String())
...
func (m *MachineInstance) Health(ctx context.Context) (*health.HealthStatus, error) {
	if !m.ConnectionAlive() {
		return nil, fmt.Errorf("connection dead - failed to connect to agent in firecracker vm: %s", m.Cfg.VmId.String())
	}
